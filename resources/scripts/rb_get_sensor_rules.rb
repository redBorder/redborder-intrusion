...
QUIET        = 0
TFLITE_MAGIC = [0x1c, 0x00, 0x00, 0x00, 0x54, 0x46, 0x4c, 0x33].freeze
...
def print_fail(text_length = 76)
  ...
  print format("%#{RES_COL - text_length}s", '[  FAILED  ]')
  puts ''
end

def get_rules(remote_name, snortrules, binding_id)
  # la actualización del fichero temporal se hace si encontró nuevas actualizaciones
  ...
  # group id es un número de grupo
...
def get_gen_msg
  ...
  v_md5sum      = File.exist?(GENFILE) ? Digest::MD5.hexdigest(File.read(GENFILE)) : ''
...
def get_classifications
  ...
  if result
    ...
    v_md5sum        = File.exist?(@v_classifications) ? Digest::MD5.hexdigest(File.read(@v_classifications)) : ''
...
def get_thresholds(binding_id)
  ...
  if result
    ...
    v_md5sum        = File.exist?(@v_threshold) ? Digest::MD5.hexdigest(File.read(@v_threshold)) : ''
...
def tensorflow_lite_model?(file_path)
  File.open(file_path, 'rb') do |f|
    ...
    exit 1 if bytes.nil? || bytes.size < TFLITE_MAGIC.size

    return true if bytes == TFLITE_MAGIC
  end
...
def get_iplist_files
  ...
  if result
    ...
    File.open("#{@v_iplist}.tmp", File::CREAT | File::TRUNC | File::RDWR, 0o755) { |f| f.write(result) }
    v_md5sum_tmp    = Digest::MD5.hexdigest(File.read("#{@v_iplist}.tmp"))
    v_md5sum        = File.exist?(@v_iplist) ? Digest::MD5.hexdigest(File.read(@v_iplist)) : ''

    if v_md5sum != v_md5sum_tmp
      system("rm -f #{@v_iplist}; mv #{@v_iplist}.tmp #{@v_iplist}; sh #{@v_iplist}")
      if File.zero?(@v_iplist_zone.to_s)
        if File.zero?(@v_iplist_zone.to_s)
          @reload_snort = 1
        else
          @restart_snort = 1
...
def get_geoip_files
  ...
  if result
    ...
    File.open("#{@v_geoip}.tmp", File::CREAT | File::TRUNC | File::RDWR, 0o755) { |f| f.write(result) }
    v_md5sum_tmp    = Digest::MD5.hexdigest(File.read("#{@v_geoip}.tmp"))
    v_md5sum        = File.exist?(@v_geoip) ? Digest::MD5.hexdigest(File.read(@v_geoip)) : ''
...
def copy_backup(backup_dir, datestr, temp_file_path, final_file_path, filename, backups)
  ...
  return unless files.size > BACKUPCOUNT
...
if Dir.exist?(@v_group_dir) && File.exist?("#{@v_group_dir}/env")
  ...
  if (@reload_snort == 1) || (@restart_snort == 1)
  ...
  binding_ids.each do |binding_id|
    ...
    if binding_id.nil? || binding_id.negative?
      print "Error: binding id not found or it is not valid\n"
    elsif dbversion_ids.nil? || dbversion_ids.empty?
      ...
      # el classification config no se está generando en intrusion, porque no existe en el cookbook de snort3
      if (@reload_snort == 1) || (@restart_snort == 1)
        datestr = Time.now.strftime('%Y%m%d%H%M%S')
        copy_backup(@v_backup_dir, datestr, "#{@v_rulefile}.tmp", @v_rulefile, @v_rulefilename, backups)
        copy_backup(@v_backup_dir, datestr, "#{@v_classifications}.tmp", @v_classifications, @v_classificationsname,
                    backups)
        copy_backup(@v_backup_dir, datestr, "#{@v_threshold}.tmp", @v_threshold, @v_thresholdname,
                    backups)
      end
      ...
      File.delete "#{@v_classifications}.tmp" if File.exist?("#{@v_classifications}.tmp")
      File.delete "#{@v_threshold}.tmp" if File.exist?("#{@v_threshold}.tmp")
      if savecmd && @group_id && !binding_id.nil? && !dbversion_ids.nil? && !dbversion_ids.empty?
        begin
  ...
  if (@reload_snort == 1) || (@restart_snort == 1)
    ...
    if system("/bin/env BOOTUP=none /usr/lib/redborder/bin/rb_verify_snort.sh #{@group_id}")
      ...
      if @reload_snort_ips == 1
        ...
        if text_return.match(/\A(ERROR: |Failed to read the response)/) && Dir.glob("/etc/snort/#{@group_id}/iplists/*.[w|b]lf").any?
